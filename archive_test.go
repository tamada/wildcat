...
func TestArchives(t *testing.T) {
	...
	for _, td := range testdata {
		argf := NewArgfWithOptions([]string{td.giveFileName}, &ReadOptions{FileList: false, NoIgnore: true, NoExtract: false}, &RuntimeOptions{ShowProgress: false, ThreadNumber: 10, StoreContent: false})
		wc := newWildcatImpl(argf.Options, argf.RuntimeOpts, DefaultGenerator)
