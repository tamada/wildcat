...
func createResultSetForTest() *ResultSet {
	ec := NewErrorCenter()
	target := NewTarget([]string{"testdata/humpty_dumpty.txt", "testdata/ja/sakura_sakura.txt"}, ec)
...
func TestXmlPrinter(t *testing.T) {
	...
	rs.Print(NewPrinter(writer, "xml"))
	result := writer.String()
	if !strings.Contains(result, `<result><file-name>testdata/humpty_dumpty.txt</file-name><lines>4</lines><words>26</words><characters>142</characters><bytes>142</bytes></result>`) {
		t.Errorf("printed xml did not contains the result of humpty_dumpty.txt, got %s", result)
	}
	if !strings.Contains(result, `<result><file-name>testdata/ja/sakura_sakura.txt</file-name><lines>15</lines><words>26</words><characters>118</characters><bytes>298</bytes></result>`) {
...
func TestDefaultPrinter(t *testing.T) {
	writer := new(strings.Builder)
	rs := createResultSetForTest()
	rs.Print(NewPrinter(writer, "unknown"))
	result := writer.String()
	if !strings.Contains(result, `4         26        142        142 testdata/humpty_dumpty.txt`) {
		t.Errorf("the result by DefaultPrinter did not contains humpty_dumpty.txt, got %s", result)
	}
	if !strings.Contains(result, `15         26        118        298 testdata/ja/sakura_sakura.txt`) {
...
func TestJsonPrinter(t *testing.T) {
	writer := new(strings.Builder)
	rs := createResultSetForTest()
	rs.Print(NewPrinter(writer, "json"))
	result := writer.String()
	if !strings.Contains(result, `{"filename":"testdata/humpty_dumpty.txt","lines":4,"words":26,"characters":142,"bytes":142}`) {
		t.Errorf("the result by JsonPrinter did not contains humpty_dumpty.txt, got %s", result)
	}
	if !strings.Contains(result, `{"filename":"testdata/ja/sakura_sakura.txt","lines":15,"words":26,"characters":118,"bytes":298}`) {
...
func TestCsvPrinter(t *testing.T) {
	writer := new(strings.Builder)
	rs := createResultSetForTest()
	rs.Print(NewPrinter(writer, "csv"))
	result := writer.String()
	if !strings.Contains(result, `testdata/humpty_dumpty.txt,4,26,142,142`) {
		t.Errorf("the result by CsvPrinter did not contains humpty_dumpty.txt, got %s", result)
	}
	if !strings.Contains(result, `testdata/ja/sakura_sakura.txt,15,26,118,298`) {
